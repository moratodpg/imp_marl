...
from pymarl.modules.critics.ddmac import DDMACCritic
...
class DDMACLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.critic = DDMACCritic(scheme, args)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        behavior =  batch["behavior"][:,:-1] # Retrieving original policies
        orig_behav = th.gather(behavior, dim=3, index=actions[:,:-1] )
        critic_mask = mask.clone()

        mask = mask.repeat(1, 1, self.n_agents).view(-1)
        
        
        current_behavior_out = [] # Retrieving current policies
        ...
        rho_clip = th.ones(rho.size())*2 # Clipping
        rho = ( th.min(rho, rho_clip) ).detach()
        v_vals, targets_adv, critic_train_stats = self._train_critic(batch, rewards, terminated, actions, avail_actions,
                                                        critic_mask, bs, max_t, rho)
        ...
        rho_taken = rho.view(-1, 1).squeeze(1)

        advantages = ((targets_adv - v_vals).squeeze(1)).detach()
        ddmac_loss = - ((advantages * log_pi_taken* rho_taken) * mask).sum() / mask.sum()
        # Optimise agents ddmac_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("ddmac_loss", ddmac_loss.item(), t_env)
            self.logger.log_stat("agent_grad_norm", grad_norm, t_env)
            self.logger.log_stat("pi_max", (pi.max(dim=1)[0] * mask).sum().item() / mask.sum().item(), t_env)
            self.log_stats_t = t_env

    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t, rho):
        # Optimise critic
        target_v_vals = self.target_critic(batch)[:, :]
        targets_taken = target_v_vals.squeeze(3)
