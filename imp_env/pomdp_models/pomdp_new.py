...
class Pomdp:
    ...
    def crack_growth(self, nsamples):
        ...
        for t in range(self.T):
            dt = (((2-self.m)/2)*C*(math.pi**0.5*S)**self.m*self.ncycles + d0**((2-self.m)/2))**(2/(2-self.m))
    ...
    def transition_model(self, n_dstates=30):
        ...
        interv_transf = np.exp( np.arange(interv_start, interv_end + interv_step, interv_step) )
