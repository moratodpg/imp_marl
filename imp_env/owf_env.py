...
class Struct_owf(ImpEnv):
    def __init__(self, config=None):
        ...
        numpy_models = np.load(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "pomdp_models/owf6021.npz"
            )
        )
    ...
    def reset(self):
        ...
        self.damage_proba = self.initial_damage_proba
        self.d_rate = np.zeros((self.n_owt, self.lev, 1), dtype=int)
        damage_proba_comp = np.reshape(
            self.damage_proba[:, :-1, :], (self.n_agents, -1)
        )
        self.observations = {}
        ...
        for i in range(self.n_owt):
            for j in range(self.lev - 1):
                if a[(self.lev - 1) * i + j] == 1 and j == 0:
                    ...
                    Bplus = self.transition_model[
                        a[(self.lev - 1) * i + j], j, drate[i, j, 0]
                    ].T.dot(B[i, j, :])
                    PF_[i] = Bplus[-1]
    ...
    def belief_update_uncorrelated(self, proba, action, drate):
        ...
        for i in range(self.n_owt):
            for j in range(self.lev - 1):
                p1 = self.transition_model[
                    action[(self.lev - 1) * i + j], j, drate[i, j, 0]
                ].T.dot(
                    new_proba[i, j, :]
                ...
                inspection[
                    i, j
                ] = 2  # ib[o] = 0 if no crack detected 1 if crack detected
                if action[(self.lev - 1) * i + j] == 1:
                    ins0 = np.sum(
                        p1
                        * self.inspection_model[action[(self.lev - 1) * i + j], j, :, 0]
                    )
                    ...
                    if ins1 < 1e-5:
                        ...
                        inspection[i, j] = np.random.choice(
                    ...
                    new_proba[i, j, :] = (
                        ...
                        * self.inspection_model[
                            action[(self.lev - 1) * i + j], j, :, int(inspection[i, j])
                        ]
                        / (
                            p1.dot(
                                self.inspection_model[
                                    action[(self.lev - 1) * i + j],
                                    j,
                                    :,
                                    int(inspection[i, j]),
                                ]
