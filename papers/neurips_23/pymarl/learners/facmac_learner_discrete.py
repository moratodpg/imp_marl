...
from modules.mixers.qmix_ablations import QMixerNonmonotonic, VDNState
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.mixer is not None:
            if self.args.mixer == "vdn":
                q_taken = self.mixer(
                    q_taken.view(-1, self.n_agents, 1), batch["state"][:, :-1]
                )
            else:
                q_taken = self.mixer(
                    q_taken.view(batch.batch_size, -1, 1), batch["state"][:, :-1]
                )
        ...
        if self.mixer is not None:
            if self.args.mixer == "vdn":
                target_vals = self.target_mixer(
                    target_vals.view(-1, self.n_agents, 1), batch["state"][:, :]
                )
            else:
                target_vals = self.target_mixer(
                    target_vals.view(batch.batch_size, -1, 1), batch["state"][:, :]
                )
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("critic_grad_norm", critic_grad_norm.cpu(), t_env)
