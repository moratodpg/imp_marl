...
class DDMACLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        ddmac_loss = (
            -((advantages * log_pi_taken * rho_taken) * mask).sum() / mask.sum()
        )
    ...
    def _train_critic(
        self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t, rho
    ):
        # Optimise critic
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            running_log["v_taken_mean"].append(
                (v_taken * mask_t).sum().item() / mask_elems
            )
