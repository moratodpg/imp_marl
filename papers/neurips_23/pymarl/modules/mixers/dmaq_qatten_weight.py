...
class Qatten_Weight(nn.Module):
    ...
    def forward(self, agent_qs, states, actions):
        ...
        all_head_keys = [
            [k_ext(enc) for enc in unit_states] for k_ext in self.key_extractors
        ]
        ...
        head_attend_logits = []
        head_attend_weights = []
        for curr_head_keys, curr_head_selector in zip(
            all_head_keys, all_head_selectors
        ):
            ...
            if self.args.mask_dead:
                ...
                scaled_attend_logits[
                    actions == 0
                ] = -99999999  # action == 0 means the unit is dead
            attend_weights = F.softmax(
                scaled_attend_logits, dim=2
            )  # (batch_size, 1, agent_num)
