...
print("max return".ljust(len(list(transposed_result.keys())[0])-15-12-6), "\t", end='')
for key in result:
...
for idx, (name, value) in enumerate(transposed_result.items()):
    ...
    try:
        with open(os.path.join(os.path.join(os.path.dirname(name), "info.json")), "r") as file:
            info = json.load(file)
            print(str(max([d["value"] for d in info["test_return_mean"]])).ljust(ljust), "\t", end='')
    except Exception as e:
        print(e)
    for key in result:
        if key in value:
