...
if __name__ == '__main__':
    ...
    # initialize env
    config_path = os.path.join(os.path.dirname(__file__), "config/impmarl.yaml")
    env = marl.make_env(environment_name="imp-marl", map_name="all_scenario", abs_path=config_path)

    algo = marl.algos.mappo(hyperparam_source="common")
    # customize model
    model = marl.build_model(env, algo, {"core_arch": "gru"})
    # start learning
    algo.fit(env, model, stop={'episode_reward_mean': -15, 'timesteps_total': 100000}, local_mode=True, num_gpus=0,
